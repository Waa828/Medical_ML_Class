...
def quadratic_bowl(x: np.ndarray) -> float:
    return 0.5 * float(x.T @ A @ x) - float(x.T @ b)

def grad_quadratic_bowl(x: np.ndarray) -> np.ndarray:
    return A @ x - b

def plot_results(history, target):
    ...
    axes[1].scatter([target[0,0]], [target[1,0]], c='red', s=80, marker='*', label='Target', zorder=5)
    ...
    axes[1].set_xlabel('X')
    axes[1].set_ylabel('Y')
    axes[1].set_title('Quadratic Bowl Contour with Training Path')
    ...
    plt.tight_layout()
    plt.savefig('P1Q22_plot_results.png', dpi=150)
    plt.show()

def analyze_initial_guess(target):
    """Analyze the effect of initial guess on convergence"""
    init_guesses = [
        (np.array([[0.], [0.]]), '(0, 0)'),
        (np.array([[20.], [20.]]), '(20, 20)'),
        (np.array([[60.], [60.]]), '(60, 60)'),
        (np.array([[-10.], [50.]]), '(-10, 50)'),
        (np.array([[50.], [-10.]]), '(50, -10)'),
    ]
    ...
    axes[1].scatter([target[0,0]], [target[1,0]], c='red', s=100, marker='*', label='Target', zorder=5)

    colors = plt.cm.tab10(np.linspace(0, 1, len(init_guesses)))

    for (init_guess, label), color in zip(init_guesses, colors):
        _, history = gd_function_with_history(quadratic_bowl, grad_quadratic_bowl, init_guess, 0.01, 1e-10, 1000)
    ...
    plt.tight_layout()
    plt.savefig('P1Q22_initial_guess_analysis.png', dpi=150)
    plt.show()

def analyze_step_size(target):
    """Analyze the effect of step size on convergence"""
    step_sizes = [0.001, 0.005, 0.01, 0.05, 0.1, 0.15]
    ...
    for step_size, color in zip(step_sizes, colors):
        result, history = gd_function_with_history(quadratic_bowl, grad_quadratic_bowl, init_guess, step_size, 1e-10, 1000)
        distance = np.linalg.norm(result - target)
        results.append((step_size, len(history['iter']), distance, history['converged']))
        axes[0].plot(history['iter'], history['grad_norm'], '-', color=color, label=f'α={step_size}', linewidth=2)
    ...
    # Bar chart: iterations and distance to extremum
    ...
    ax2.set_xticklabels([f'{s}' for s in step_sizes])
    ax2.set_xlabel('Step Size (α)')
    ax2.set_title('Iterations and Distance to Extremum vs Step Size')

    plt.tight_layout()
    plt.savefig('P1Q22_step_size_analysis.png', dpi=150)
    plt.show()
    ...
    for step_size, iters, dist, converged in results:
        ...
        print(f"{step_size:>12.4f} {iter_str:>12} {dist:>20.6f}")

def analyze_epsilon(target):
    """Analyze the effect of convergence criterion on final solution"""
    epsilons = [1e-3, 1e-5, 1e-8, 1e-10, 1e-15]
    init_guess = np.array([[0.], [0.]])
    ...
    for epsilon, color in zip(epsilons, colors):
        result, history = gd_function_with_history(quadratic_bowl, grad_quadratic_bowl, init_guess, 0.01, epsilon, 2000)
        error = np.linalg.norm(result - target)
        results.append((epsilon, len(history['iter']), error, result, history))

    x = np.linspace(39.95, 40.05, 100)
    y = np.linspace(39.95, 40.05, 100)
    X, Y = np.meshgrid(x, y)
    ...
    ax.scatter([target[0,0]], [target[1,0]], c='red', s=100, marker='*', label='Target', zorder=5)
    ...
    plt.tight_layout()
    plt.savefig('P1Q22_epsilon_analysis.png', dpi=150)
    plt.show()

    print("\nConvergence Criterion Analysis Results:")
    print("-" * 60)
    print(f"{'ε':>12} {'Iterations':>12} {'Distance to Target':>15}")
    print("-" * 60)
    for epsilon, iters, error, result, history in results:
        print(f"{epsilon:>12.0e} {iters:>12} {error:>15.6e}")
...
if __name__ == '__main__':
    """
    P1Q22: Quadratic Bowl Optimization with Parameter Sensitivity Analysis
    
    Objective: Minimize quadratic function f(x) = 0.5*x^T*A*x - b^T*x
    Target: x* = [40, 40] (analytical solution from A^-1 * b)
    ...
    Workflow:
        ...
        5. Parameter sweep and export results to CSV
    
    Hyperparameters: step_size=0.01, epsilon=1e-10, max_iter=1000
    Output: PNG plots and P1Q22_results.csv
    """
    ...
    print("=" * 60)
    result, history = gd_function_with_history(quadratic_bowl, grad_quadratic_bowl, np.array([[0.], [0.]]), 0.01, 1e-10, 1000)
    print("Optimal point:", result.T)
    print("Function value at optimal point:", quadratic_bowl(result))
    print("Expected:", target.T)
    plot_results(history, target)
    ...
    analyze_initial_guess(target)

    print("\n2. Analyzing step size effect...")
    analyze_step_size(target)

    print("\n3. Analyzing convergence criterion effect...")
    analyze_epsilon(target)
    ...
    run_parameter_sweep_and_export(
        quadratic_bowl, grad_quadratic_bowl,
        init_guesses, step_sizes, epsilons,
        max_iter=1000,
        filename='P1Q22_results.csv'
    )
